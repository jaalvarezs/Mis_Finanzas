...
function initTheme() {
    ...
    document.getElementById('metaTheme').setAttribute('content', savedTheme === 'dark' ? '#0B1121' : '#f1f5f9');
}
...
function toggleTheme() {
    ...
    document.getElementById('metaTheme').setAttribute('content', newTheme === 'dark' ? '#0B1121' : '#f1f5f9');
}
...
function obtenerIcono(categoria, tipo) {
    ...
    if(text.includes('suscripción') || text.includes('gemini')) return '🤖';
    return '📝';
}
...
async function cargarDatos() {
    ...
    const datosGuardados = localStorage.getItem("finanzas_cache_v11");
    if (datosGuardados) {
    ...
    if (!data.error) { 
        localStorage.setItem("finanzas_cache_v11", JSON.stringify(data));
        estadoApp = {
            creditos: data.creditos || [], tarjetas: data.tarjetas || [],
            comparativoMensual: data.comparativoMensual || [], categoriasGastos: data.categoriasGastos || [],
            categoriasAhorro: data.categoriasAhorro || [], ultimosMovimientos: data.ultimosMovimientos || []
...
function pintarDashboard(data) {
    ...
    document.getElementById("dashboard-content").innerHTML = `
        ...
        <div class="stats">
            <div class="stat"><span class="stat-icon">⬆️</span><div class="label" style="margin:0;">Ingresos</div><div class="stat-value green">${formatoPesos(mes.ingresos)}</div></div>
            <div class="stat"><span class="stat-icon">⬇️</span><div class="label" style="margin:0;">Gastos</div><div class="stat-value red">${formatoPesos(mes.gastos)}</div></div>
            <div class="stat" style="grid-column: span 2;"><span class="stat-icon">💰</span><div class="label" style="margin:0;">Ahorro</div><div class="stat-value blue">${formatoPesos(mes.ahorro)}</div></div>
        </div>
    `;
}
...
function pintarDistribucion(idContenedor, lista, claseBarra) {
    ...
    lista.forEach(item => {
        ...
        div.innerHTML = `
            <div class="bar-head">
                <span style="color: var(--text-main);">${icono} ${item.categoria} <span style="font-size: 11px; color: var(--text-muted);">(${porcentaje}%)</span></span>
...
function lanzarConfeti() {
    const colores = ['#10b981', '#0ea5e9', '#f59e0b', '#ef4444', '#8b5cf6'];
    for (let i = 0; i < 110; i++) {
...
function pintarMetaAhorro() {
    ...
    cont.innerHTML = `
        <div class="stats" style="margin-bottom: 15px;">
            <div class="stat"><div class="label" style="margin:0;">Ahorro Ideal (5%)</div><div class="stat-value blue">${formatoPesos(metaTotal)}</div></div>
            <div class="stat"><div class="label" style="margin:0;">Ahorro Real</div><div class="stat-value ${diffTotal < 0 ? 'red' : 'green'}">${formatoPesos(totalAhorro)}</div></div>
        </div>
        
        <div style="text-align: center; background: var(--input-bg); border-radius: 12px; padding: 12px; border: 1px solid var(--card-border);">
            <div style="font-size: 13px; color: var(--text-muted); text-transform: uppercase; font-weight: 800;">Estado de la Meta</div>
            <div class="money-big ${diffTotal < 0 ? 'red' : 'green'}" style="font-size: 24px;">
                ${diffTotal < 0 ? '⚠️ Deuda: ' : '✅ Favor: '} ${formatoPesos(Math.abs(diffTotal))}
            </div>
        </div>
        ...
        <div style="background: var(--input-bg); padding: 20px 15px; border-radius: 16px; border: 1px solid var(--card-border);">
...
function pintarModuloCreditos() {
    ...
    estadoApp.creditos.forEach(c => { 
        html += `<div class="card" style="padding: 15px;"><div class="debt-title"><span>🏦 ${c.nombre}</span><span class="red">${formatoPesos(c.saldoActual)}</span></div><div class="movement-bottom">${c.entidad} · Cuota: <strong class="text-main">${formatoPesos(c.cuotaActual)}</strong></div></div>`; 
    });
    estadoApp.tarjetas.forEach(t => { 
        html += `<div class="card" style="padding: 15px;"><div class="debt-title"><span>💳 ${t.nombre}</span><span class="red">${formatoPesos(t.saldoActual)}</span></div><div class="movement-bottom">Cupo: ${formatoPesos(t.cupoTotal)} · Disp: <strong class="green">${formatoPesos(t.disponible)}</strong></div></div>`; 
    });
    cont.innerHTML = html;
}
...
function pintarHistorial() {
    ...
    estadoApp.ultimosMovimientos.forEach(m => {
        ...
        <div class="historial-item">
            <div class="h-icon" style="color: var(--${colorClase});">${icono}</div>
            <div class="h-details">
                <div class="h-title">${m.concepto}</div>
                <div class="h-date">${m.fechaTexto} · ${m.categoria}</div>
            </div>
